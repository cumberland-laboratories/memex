...
- `SYSTEM_PROMPT` — hardcoded system prompt injected as PINNED. Instructs the model to work step by step and not call tools in its final response.
...
### __init__(client, context, session, max_iterations=20)
...
! Escalation is stubbed — returns a string, doesn't actually ask the user. See → [Ask vs Act Thresholds](../active-threads/ask-vs-act-thresholds.md)
...
! Errors become strings, not exceptions — the model sees the error in its next turn and can react. This is intentional: → [Error Recovery as Design](../active-threads/error-recovery-as-design.md)
...
! This is deliberately naive — a real implementation needs semantic comparison, not exact match. See → [Agentic Loop Failure Modes](../active-threads/agentic-loop-failure-modes.md)
