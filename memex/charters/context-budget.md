...
- `MODEL_MAX_TOKENS` [L18] — dict mapping model names to max context. Currently only claude-sonnet-4-20250514 and claude-haiku-4-20250414 at 200K.
- `DEFAULT_MAX_TOKENS` [L19] — 200,000. Fallback if model not in dict.
- `OUTPUT_RESERVE` [L20] — 8,192 tokens reserved for model output. Subtracted from budget.
- `RECENT_TURN_COUNT` [L21] — 6 turns kept as RECENT before demotion to HISTORICAL.

## Priority (IntEnum) [L24]
...
## ContextManager class [L41]

### __init__(model="claude-sonnet-4-20250514") [L44]
...
### add(role, content, priority=RECENT) [L49]
